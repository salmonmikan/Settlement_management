...
<%@ page import="bean.TransportType" %>
<%@ page import="java.util.List" %>
<%
  List<TransportType> transportList = (List<TransportType>) request.getAttribute("transportList");
%>
<%
    ...
    <form action="<%= request.getContextPath() %>/businessTrip" method="post" enctype="multipart/form-data">
      ...
      <div style="display: flex; flex-direction: column; gap: 10px" id="trans-container">
        <div class="form-section trans-block">
          ...
          <div class="form-group"><label>訪問月・日</label><input type="text" name="transDate[]"></div>
			  ...
			  <select name="transport[]">
			    <option value="">選択してください</option>
				    <% if (transportList != null) {
				         for (TransportType t : transportList) { %>
	            <option value="<%= t.getTransportTypeName() %>"><%= t.getTransportTypeName() %></option>
	    <%   }
	       }
	    %>
  </select>
          ...
          <div class="form-group"><label>負担者</label>
            <select name="burden3[]" onchange="calcFareTotal(this)">
              <option value="">選択してください</option>
