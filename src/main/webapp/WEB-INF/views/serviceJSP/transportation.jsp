...
<script
	src="${pageContext.request.contextPath}/static/js/transpotation.js"></script>
<style>
...
<body>
	<div class="page-container">
		<div class="content-container">
			...
			<c:if test="${not empty successCount && successCount > 0}">
				<p style="color: red; text-align: center">${successCount}
					件の申請を登録しました。</p>
			</c:if>

			<%-- Action URL cần được xác nhận lại từ TransportationServlet --%>
			<form action="<%=request.getContextPath()%>/transportationRequest"
				...
				<div style="display: flex; flex-direction: column; gap: 10px"
					...
					<c:forEach var="detail" items="${transportationApp.details}"
						varStatus="loop">
						<div class="form-section transportation-block"
							style="position: relative;">
							<button type="button" class="remove-btn"
								onclick="removeTransportationBlock(this)">×</button>
							...
							<div class="form-group">
								<label>訪問先</label> <input type="text" name="destination[]"
									placeholder="例: ABC株式会社" value="${detail.destination}" required>
							</div>
							<div class="form-group">
								<label>訪問月・日</label> <input type="date" name="date[]"
									value="${detail.date}" required>
							</div>
							<div class="form-group">
								<label>出発</label><input type="text" name="departure[]"
									placeholder="例:東京駅" value="${detail.departure}">
							</div>
							<div class="form-group">
								<label>到着</label><input type="text" name="arrival[]"
									placeholder="例:大阪駅" value="${detail.arrival}">
							</div>
							<div class="form-group">
								<label>交通機関</label> <select name="transport[]">
									<option value="">選択してください</option>
									<option value="新幹線"
										${'新幹線' == detail.transport ? 'selected' : ''}>新幹線</option>
									<option value="電車"
										${'電車' == detail.transport ? 'selected' : ''}>電車</option>
									<option value="タクシー"
										${'タクシー' == detail.transport ? 'selected' : ''}>タクシー</option>
									<option value="飛行機"
										${'飛行機' == detail.transport ? 'selected' : ''}>飛行機</option>
									<option value="自家用車"
										${'自家用車' == detail.transport ? 'selected' : ''}>自家用車</option>
									<option value="レンタカー"
										${'レンタカー' == detail.transport ? 'selected' : ''}>レンタカー</option>
									<option value="他の"
										${'他の' == detail.transport ? 'selected' : ''}>他の</option>
								</select>
							</div>
							<div class="form-group">
								<label>金額（税込）</label><input type="number" name="fareAmount[]"
									step="100" value="${detail.fareAmount}">
							</div>
							<div class="form-group">
								<label>区分</label> <select name="transTripType[]">
									<option value="">選択してください</option>
									<option value="片道"
										${'片道' == detail.transTripType ? 'selected' : ''}>片道</option>
									<option value="往復"
										${'往復' == detail.transTripType ? 'selected' : ''}>往復</option>
								</select>
							</div>
							<div class="form-group">
								<label>負担者</label> <select name="burden[]">
									<option value="">選択してください</option>
							...
							<div class="form-group">
								<label>合計</label><input type="number" name="expenseTotal[]"
									value="${detail.expenseTotal}" readonly>
							</div>
							<div class="form-group">
								<label>摘要</label>
							...
							<div class="form-group">
								<label>報告書</label>
								<textarea name="report[]" placeholder="報告書を書いてください">${detail.report}</textarea>
							</div>
							<div class="form-group">
								<label>領収書添付（交通費）</label> <input type="file"
									name="receipt_transportation_${loop.index}" multiple
				...
				<div style="text-align: center; margin-top: 10px;">
					<button type="button" class="plus-btn"
						onclick="addTransportationBlock()">＋</button>
				</div>
	...
	<template id="transportation-template">
		<div class="form-section transportation-block"
			...
			<div class="form-group">
				<label>訪問月・日</label> <input type="date" name="date[]" required>
			</div>
			<div class="form-group">
				<label>訪問先</label> <input type="text" name="destination[]"
					placeholder="例: ABC株式会社" value="${detail.destination}" required>
			...
			<div class="form-group">
				<label>合計</label><input type="number" name="expenseTotal[]" readonly>
			</div>
			<div class="form-group">
				<label>摘要</label>
			...
			<div class="form-group">
				<label>報告書</label>
				<textarea name="report[]" placeholder="報告書を書いてください"></textarea>
			</div>
			<div class="form-group">
				<label>領収書添付（交通費）</label> <input type="file"
					name="receipt_transportation_" multiple class="fileInput"
